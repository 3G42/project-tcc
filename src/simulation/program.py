...
def with_gd_storage_simulation(dss, storages):
    ...
    for storage in storages:
        dss.text(
            f"New Storage.Bateria{storage[0]} bus=p{storage[0]}.1.2.3.4 phases=3 kv=0.22 conn=wye kwrated={storage[1]} kwhrated={storage[2]} %stored=0.0 dispmode=follow daily=CurvaBAT")
        buses_with_storage.append(f"Bateria{storage[0]}")
    dss.solution.solve()
...
def solve_simulation(dss, option="without-gd-storage", storage_specs=[]):
...
def programa(option="with-gd-without-storage", storage_specs=[]):
    '''Função principal que executa a simulação e retorna os dados necessários.

    :param option: Opção de simulação, pode ser "without-gd-storage", "with-gd-without-storage" ou "with-gd-storage".
    :param storage_specs: Lista de especificações de armazenamento, necessária apenas para a opção "with-gd-storage".


    :returns: Dicionário contendo os resultados da simulação, incluindo tensões, potências
    e indicadores de qualidade de tensão.

    '''
    dss = pre_solve()
    data = solve_simulation(dss, option, storage_specs)
