...
class Individuo:
    def __init__(self, n_bess, pot_min=POT_MIN, pot_max=POT_MAX):
        self._bess_list = []
        self._n_bess = n_bess
    ...
    def to_list(self):
        ...
        while len(list_of_bess) < self._n_bess:
            list_of_bess.append([0, 0, 0])
        return list_of_bess
...
def cria_individuo(battery_qtt):
    num_of_bess = battery_qtt
...
def avaliar(individuo):
    ...
    fitness = drcs.sum() * 20 + drps.sum() * 5  # penalização aumentada
    return (fitness,)
...
def crossover_sanitizado(ind1, ind2, battery_qtt):
    i1, i2 = tools.cxTwoPoint(ind1, ind2)
    i1 = sanitizar_individuo(i1, BARRAS_CANDIDATAS, POT_MIN, POT_MAX, battery_qtt)
    i2 = sanitizar_individuo(i2, BARRAS_CANDIDATAS, POT_MIN, POT_MAX, battery_qtt)
    return i1, i2


def mutacao_customizada(individuo, indpb=0.3, battery_qtt=1):
    for bess in individuo:
        if bess[0] in BARRAS_CANDIDATAS:
            ...
            if random.random() < indpb:
                max_pot_of_bess = POT_MAX / battery_qtt
                bess[1] = float(np.clip(bess[1] + random.gauss(0, 15), POT_MIN, max_pot_of_bess))
            if random.random() < indpb:
                max_energy_of_bess = POT_MAX * 3 / battery_qtt
                bess[2] = float(np.clip(bess[2] + random.gauss(0, 30), POT_MIN * 3, max_energy_of_bess))
    individuo = sanitizar_individuo(individuo, BARRAS_CANDIDATAS, POT_MIN, POT_MAX, battery_qtt)
    return (individuo,)
...
def start_toolbox():
    ...
    toolbox.register("individual", lambda: cria_individuo(battery_qtt))
    toolbox.register("population", tools.initRepeat, list, toolbox.individual)
    toolbox.register("mate", lambda ind1, ind2: crossover_sanitizado(ind1, ind2, battery_qtt))
    toolbox.register("mutate", lambda ind: mutacao_customizada(ind, indpb=0.3, battery_qtt=battery_qtt))

    return toolbox


# === Execução ===
def rodar_ga(geracoes=20, tamanho_pop=100, battery_qtt=1):



    toolbox = start_toolbox()
...
if __name__ == "__main__":
    battery_qtt = int(input("Digite a quantidade de baterias a instalar: "))
    melhor = rodar_ga(geracoes=50, tamanho_pop=20, battery_qtt=battery_qtt)
    ...
    vb = resultado["vb"]
    vc = resultado["vc"]
    resultado_gd_sem_storage = programa(option="with-gd-without-storage")
