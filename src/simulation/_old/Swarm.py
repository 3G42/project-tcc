...
class Swarm:
    ...
    @staticmethod
    def gerar_binario_baterias(num_barras, num_baterias):

        print(num_baterias)
        posicoes = random.sample(range(1,num_barras), num_baterias)
    ...
    @staticmethod
    def gerar_particula(num_barras, num_baterias, pot_total_min, pot_total_max, min_const, max_const):
        pot_total = random.randint(pot_total_min, pot_total_max)
        binario = Swarm.gerar_binario_baterias(num_barras, num_baterias)
        # Distribui potências aleatórias, mantendo soma = pot_total_max
        pot_aleatorias = Swarm.distribuir_potencia_aleatoria(pot_total, num_baterias)
        potencias = []
        idx_pot = 0
        for b in binario:
            if b == 1:
        ...
        part = Particle(potencias, constante_energia,False,pot_total,num_baterias)
        return part
    ...
    @classmethod
    def gerar_enxame_aleatorio(cls, num_particulas, num_barras, num_baterias, pot_total_max, min_const, max_const):
        particles = [
            cls.gerar_particula(num_barras, num_baterias,2, pot_total_max, min_const, max_const)
