...
### Chamando o dss file ###
file_dss = os.path.join(os.path.dirname(__file__), 'circbtfull_storage.dss') ## Chama o dss principal usando a os
file_dss_loadshapes = os.path.join(os.path.dirname(__file__), 'loadshapes.dss') ## Chama o dss principal usando a os
...
plt.figure(figsize=(6, 6))
plt.step(df2['tempo'], df2['kW_total'], color='blue')
plt.axhspan(-100, 0, color='lightcoral', alpha=0.3)
plt.axhspan(0, 150, color='lightgreen', alpha=0.3)
plt.xlabel('Horário (h)', fontsize=15)
...
axs[0, 1].axhspan(-1.0, 0, color='lightcoral', alpha=0.3)
axs[0, 1].axhspan(0, 1.0, color='lightgreen', alpha=0.3)
...
axs[1, 1].axhspan(-1.0, 0, color='lightcoral', alpha=0.3)
axs[1, 1].axhspan(0, 1.0, color='lightgreen', alpha=0.3)
...
fig.update_layout(
    ...
    xaxis_title=dict(
        text='horário (h)',
        font=dict(size=18)  # Altere o valor de 'size' para o tamanho desejado
    ),
    yaxis_title=dict(
        text='Tensão (V)',
        font=dict(size=18)  # Altere o valor de 'size' para o tamanho desejado
    ...
    xaxis=dict(
        ...
        tick0=0,
        dtick=2,
        tickfont=dict(size=16)  # Altere o valor de 'size' para o tamanho desejado
    ),
    yaxis=dict(
        range=[100, 140],
        tickfont=dict(size=16)  # Altere o valor de 'size' para o tamanho desejado
    ),
    legend=dict(font=dict(size=16), orientation='h', x=0.5, xanchor='center', y=-0.2),
    margin=dict(l=40, r=40, t=40, b=40)
)
# Salvando a figura em HTML
fig.write_html("grafico_tensoes_barras_casen.html")
